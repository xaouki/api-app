...
def handle_sms_flow():
    ...
    try: choice = int(input("\nEnter the number of the provider: ").strip())
    except ValueError: print("❌ Please enter a valid number."); return
    if choice not in SMS_PROVIDERS: print("❌ Invalid selection!"); return
    p = SMS_PROVIDERS[choice]; credentials = {}
    for field in p["fields"]:
        val = input(f"• Enter {p['labels'][field]}: ").strip()
        if not val: print("❌ This field is required."); return
        credentials[field] = val
    from_number = input("• Enter sender number/name: ").strip()
    message_text = input("• Enter message text: ").strip()
    recipients = [x.strip() for x in input("• Enter recipient numbers separated by commas: ").split(",") if x.strip()]
    if not from_number or not message_text or not recipients: print("❌ Incomplete sending data."); return
    print(f"\n📋 {p['name']} | Sender: {from_number} | Recipients: {len(recipients)}")
    if input("Start sending now? (y/n): ").strip().lower() != "y": return
    ok = fail = 0
    for i, recipient in enumerate(recipients, 1):
        ...
        time.sleep(0.4)
    print(f"\n📊 Result: {ok} successful | {fail} failed")

def handle_email_flow():
    print("\n--- 📧 CasaSender Email Engine (SMTP) ---")
    smtp_server = input("• Enter SMTP server address: ").strip()
    try: smtp_port = int(input("• Enter port (587 or 465): ").strip())
    except ValueError: print("❌ Port must be a number."); return
    sender_email = input("• Enter sender email: ").strip()
    sender_password = input("• Enter password or App Password: ").strip()
    sender_name = input("• Enter sender name: ").strip()
    subject = input("• Enter email subject: ").strip()
    body = input("• Enter message body: ").strip()
    recipients = [x.strip() for x in input("• Enter recipient emails separated by commas: ").split(",") if x.strip()]
    if not smtp_server or not sender_email or not sender_password or not recipients: print("❌ Incomplete email data."); return
    ok = fail = 0
